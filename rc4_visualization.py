...
class LogManager:
    ...
    def log_ksa_step(self, i, j, N, key_idx):
        ...
        self.log(f"  i={i:3d}: j = (j + S[{i}] + K[{key_idx}]) mod {N} = {j:3d}, swap S[{i}]↔S[{j}]")
    ...
    def log_prga_step(self, step_num, result, plain_byte, cipher_byte, printable_char):
        ...
        self.log(f"Ciphertext[{step_num - 1}] = {plain_byte} ⊕ {result['output_byte']} = {cipher_byte} (0x{cipher_byte:02x})")
    ...
    def _log_rc4plus_step(self, result):
        ...
        self.log(f"t_prime = (S[idx1] + S[idx2]) mod 256 XOR 0xAA = {result['t_prime']}")
        self.log(f"t_double = (j + S[j]) mod 256 = {result['t_double']}")
        self.log(f"Output = ((S[t] + S[t_prime]) mod 256) XOR S[t_double] = {result['output_byte']}")
