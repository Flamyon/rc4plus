...
class ControlPanel:
    ...
    def _setup_controls(self):
        ...
        for value in [64, 128, 256, 512]:
            r = ttk.Radiobutton(
                size_frame, text=str(value), variable=self.size_var, value=value
            )
            r.pack(side=tk.LEFT)
            self.size_radios.append(r)

        # Key input
        ttk.Label(self.frame, text="Clave:").grid(row=1, column=0, padx=5, sticky=tk.W)
        ...
        self.algorithm_combo.bind(
            "<<ComboboxSelected>>", lambda e: self.on_algorithm_change()
        )
...
class ResultPanel:
    ...
    def update_results(self, plaintext="", keystream=None, ciphertext=None):
        ...
        try:
            ...
            cipher_ascii = "".join(
                [chr(b) if 32 <= b < 127 else "." for b in ciphertext]
            )
...
class StateVariablesPanel:
    ...
    def _setup_labels(self):
        ...
        self.var_i_label = ttk.Label(
            self.frame, text="i = 0", font=("Courier", 12, "bold")
        )
        self.var_i_label.pack(side=tk.LEFT, padx=10)

        self.var_j_label = ttk.Label(
            self.frame, text="j = 0", font=("Courier", 12, "bold")
        )
        self.var_j_label.pack(side=tk.LEFT, padx=10)

        self.output_label = ttk.Label(
            self.frame, text="Output = -", font=("Courier", 12, "bold")
        )
