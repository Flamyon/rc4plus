...
class RC4Visualizer:
    ...
    def setup_ui(self):
        ...
        self.control_panel = ControlPanel(
            self.root,
            on_algorithm_change_callback=self.on_algorithm_change
        )
    ...
    def _setup_left_panel(self, parent):
        ...
        scrollbar_y = ttk.Scrollbar(canvas_frame, orient=tk.VERTICAL, 
                                    command=self.state_canvas.yview)
        scrollbar_x = ttk.Scrollbar(left_frame, orient=tk.HORIZONTAL, 
                                    command=self.state_canvas.xview)
        
        self.state_canvas.configure(yscrollcommand=scrollbar_y.set, 
    ...
    def _setup_right_panel(self, parent):
        ...
        log_text = scrolledtext.ScrolledText(log_frame, width=50, height=20,
    ...
    def init_ksa(self):
        ...
        for i, j, description in ksa_steps:
            ...
            self.state_visualizer.draw_state(
                self.engine.S,
                highlights={'i': i, 'j': j}
            )
    ...
    def step_prga(self):
        ...
        if self.engine is None or not self.engine.S:
            self.log_manager.log("ERROR: Primero debe ejecutar KSA (Inicializar)", "red")
            return
    ...
    def run_rc4plus_test(self):
        ...
        if recovered_text == 'Plaintext':
            self.log_manager.log('✓ Test PASSED: Encryption/Decryption is symmetric', 'green')
        else:
